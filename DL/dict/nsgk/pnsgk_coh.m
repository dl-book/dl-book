...
function [D,X,shared] = pnsgk_coh(Y,D,X,iter,replatoms,shared,varargin)
%% P-NSGK algorithm with coherence reduction
% INPUTS:
...
%   D -- updated dictionary
    ...
    [D,X] = pnsgk_loop(Y,D,X,iter,replatoms, pa, ...
        @(F,D,d,x) coh_up(F,D,d,x,gamma));
end

function [d,x] = coh_up(F,D,d,x,gamma)
%% P-NSGK algorithm with coherence reduction atom update
% INPUTS:
