...
function [D,X,shared] = nsgk_reg(Y,D,X,iter,replatoms,shared,varargin)
%% NSGK algorithm with regularization
% INPUTS:
...
%   D -- updated dictionary
    ...
    [D,X] = nsgk_loop(Y,D,X,iter,replatoms,@(F,D,d,x) nsgk_up(F,x));
end

function [d,x] = nsgk_up(F,x)
%% NSGK with regularization atom update
% INPUTS:
