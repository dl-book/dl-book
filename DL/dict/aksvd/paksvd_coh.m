...
function [D,X,shared] = paksvd_coh(Y,D,X,~,replatoms,shared,varargin)
%% PAK-SVD algorithm with coherence reduction
% INPUTS:
...
%   X -- updated representations
    ...
    [D,X] = patom_up(Y,D,X,replatoms,pa,@(F,D,d,x) coh_up(F,D,d,gamma));
end

function [d,x] = coh_up(F,D,d,gamma)
%% PAK-SVD algorithm with coherence reduction atom update
% INPUTS:
...
%   x -- updated representations corresponding to the current atom
    ...
    x = F'*d;
    % Update atom
    d = F*x - 2*gamma*D*(D'*d) + 2*sqrt(size(D,1))*gamma*d;
    d = d/norm(d);
    x = F'*d;
end