...
function [D,X,shared] = psgk_reg(Y,D,X,iter,replatoms,shared,varargin)
%% P-SGK algorithm with regularization
% INPUTS:
...
%   D -- updated dictionary
    ...
    X = ompreg(Y,D,X,mu);
    [D,X] = patom_up(Y,D,X,replatoms,pa,@(F,D,d,x) reg_up(F,x));
end

function [d,x] = reg_up(F,x)
