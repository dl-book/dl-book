...
function [D,X,shared] = sgk_coh(Y,D,X,~,replatoms,shared,varargin)
%% SGK algorithm with coherence reduction
% INPUTS:
...
%   D -- updated dictionary
    ...
    [D,X] = atom_up(Y,D,X,replatoms,@(Y,D,X,d,x) coh_up(Y,D,X,d,x,gamma));
end

function [d,x] = coh_up(Y,D,X,d,x,gamma)
%% SGK with coherence reduction atom update
% INPUTS:
%   Y -- training signals set
%   D -- current dictionary
%   X -- sparse representations
...
%   d -- updated atom
    ...
    % Coherence reduction
    d = Y * x0 - D*(X*x0) - 2*gamma*D*(D'*d) + 2*sqrt(size(D,1))*gamma*d;
    d = d/norm(x0)^2;
